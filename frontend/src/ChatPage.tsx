...
export default function ChatPage() {
  ...
  async function loadConversation(id: number) {
    ...
    setCurrentConvId(c.temporary ? c.id : null)
    setCandidates(null); setStreaming(null); setStage(null)
  }
  function newChat() {
    setMessages([]); tempId.current = null; setCurrentConvId(null)
    setCandidates(null); setCandDone({}); setStreaming(null); setStage(null); setInput("")
  }

  // 接回存下的對話（?resume=id，從別處連來）
  useEffect(() => {
    const rid = Number(sp.get("resume") || 0)
    if (!rid) return
    loadConversation(rid).finally(() => { sp.delete("resume"); setSp(sp, { replace: true }) })
    ...
    await streamChat(hist, msg, brainstorm, {
      ...
      onDone: (text, sources, extra) => {
        ...
        api.autosave(next, tempId.current).then((r) => {
          tempId.current = r.temp_id; setCurrentConvId(r.temp_id); setHistoryRefresh((n) => n + 1)
        }).catch(() => {})
      },
    })
    setBusy(false)
  ...
  async function anointOne(i: number, c: Candidate) {
    ...
    if (saveConvo) setHistoryRefresh((n) => n + 1)   // 連同存對話→歷史更新
  }
  ...
  async function saveConversation() {
    ...
    if (r.saved) { tempId.current = null; setCurrentConvId(null); setHistoryRefresh((n) => n + 1) }
    toast(r.msg)
  }
  ...
  const history = (onClose?: () => void) => (
    <ConversationHistory
      currentId={currentConvId} onResume={loadConversation} onNew={newChat}
      refreshKey={historyRefresh} onClose={onClose}
    />
  )
  ...
  return (
    <div className="flex h-full">
      {/* 桌面：固定歷史側欄 */}
      <aside className="hidden w-64 shrink-0 border-r bg-sidebar md:block">{history()}</aside>
      {/* 手機：抽屜 */}
      {historyOpen && (
        <div className="fixed inset-0 z-40 flex md:hidden">
          <div className="absolute inset-0 bg-black/30" onClick={() => setHistoryOpen(false)} />
          <aside className="relative z-10 w-72 max-w-[80%] bg-sidebar shadow-xl">{history(() => setHistoryOpen(false))}</aside>
        </div>
      )}

      {/* 聊天區 */}
      <div className="mx-auto flex h-full max-w-3xl flex-1 flex-col px-4 py-3 md:px-8">
        <div className="flex shrink-0 items-center gap-2 pb-2">
          <button className="rounded-md px-1 text-lg md:hidden" onClick={() => setHistoryOpen(true)} aria-label="歷史對話">☰</button>
          <div>
