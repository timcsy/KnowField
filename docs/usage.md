...
KnowField 是**消化＋溯源工具**：把 AI 新聞與論文的洪流去重、依你的興趣排序、**消化成
...
uv run knowfield digest        # 先產一份匯整（首頁會讀它）
# 導覽列：今日匯整 / 主題深挖 / 搜尋 / 問答 / 收進 / 知識庫 / 根因 / 來源 / 興趣
uv run uvicorn knowfield.web.app:app --reload   # 開 http://127.0.0.1:8000
```

首頁看今日匯整——**分兩區（階段 14）**：**📰 今日新聞**（剛紅的趨勢＝流，會更替：新聞媒體／
HN／Reddit／web 活水）＋**📚 基礎知識精選**（常青的底子＝吸引子：arXiv 論文／教學／經典部落格）。
這是把 concept「流 vs 吸引子」在 UI 顯性化；空的那區不顯示。上方輸入框即時「拉」主題、`/interests`
...
高頻」非預言）；無匯整/算不出則不顯示。`KNOWFIELD_TREND_TOPN`（8）、`KNOWFIELD_TREND_RECENT`（3）。RWD（手機/桌面）、全繁中；後端失敗會顯示友善頁面而非錯誤堆疊。框架相依只在
web 這層，核心仍零相依。
...
結果。真搜需設 `KNOWFIELD_SEARCH_API_URL`＋`KNOWFIELD_SEARCH_KEY`（Tavily，金鑰放 body）；
未設走離線示意。前 N 可調 `KNOWFIELD_SMART_TOPN`（預設 4）。

**場對新材料做工（階段 15）＝護城河核心**：在「知識庫」對一則種子按「🧭 關聯到我的場」→ 工具
把它**接到你冊封的根因/種子**上：**延伸**你哪個根因、**牴觸**你冊封的哪個（← Google 不會告訴你
「這跟你以為的相反」）、還是離你所有吸引子都遠（**成核候選**＝新地基，建議萃取根因/收進）。關係
**grounded**（只依材料＋根因、不杜撰、無關說無關）、可回核對；**AI 只提關係、你決定——場不自動
改**（原則 5）。這是通用搜尋做不到的「只對你成立的連結」。未設金鑰走離線示意。
...
提候選、**人冊封才算數**（不自動冊封）；`KNOWFIELD_RAG_ROOT_WEIGHT`（預設 2.0）。

**深入探索（多角度，opt-in／預設關）**：勾「深入探索」時，工具把 query 拆成 3–5 個不同角度
各搜一次、**合併去重**，再跑同一套整理——撒更廣的網、照到單一問法的盲區。**預設關**，貴的
（多次搜尋＋一次拆解）只在你要時才花；子角度上限 `KNOWFIELD_EXPLORE_MAXQ`（預設 5），抓取
...
uv run knowfield interests set "LLM 推理" "agent" "編譯器"

# 2. 產出今天的分診匯整
uv run knowfield digest --date 2026-07-23 --limit 15
```
...
| 指令 | 說明 |
|---|---|
| `knowfield digest [--date D] [--limit N] [--format terminal\|markdown] [--output PATH] [--json]` | 產出當日匯整（推模式：分診） |
| `knowfield pull <主題> [--limit N] [--raw] [--from-digest N] [--format ...] [--json]` | 對主題擴展、去重、溯源（拉模式：深挖） |
| `knowfield ask "<問題>" [--today] [--lang L] [-k N]` | 對已落庫知識庫問答（RAG，可溯源） |
| `knowfield ingest <arXiv-id\|url> [--explainer]` | 手動把一篇經典/解說文收進知識庫（種子） |
| `knowfield interests list\|add <主題>\|remove <主題>\|set <主題...>` | 管理興趣清單（明講優先於學習） |
| `knowfield sources list\|enable <id>\|disable <id>` | 檢視／啟用來源 |
...
（非 SEO 常青清單文）——`KNOWFIELD_SEARCH_NEWS_RANGE`（預設 `week`，可 `day`/`month`）。手動
`/search` 維持一般搜尋（不限新聞、不限時間）。

### 問答（RAG，對累積的匯整發問）

```bash
uv run knowfield ask "最近 agent 記憶體有什麼進展"   # 對所有已落庫匯整檢索、合成可溯源答案
uv run knowfield ask "今天的重點" --today            # 只查最近一份匯整
uv run knowfield ask "RL 新方向" -k 8                 # 取回上限 8 則
uv run knowfield ask "..." --lang English            # 答案語言（預設繁體中文）
```
...
```bash
uv run knowfield ingest 1706.03762 --explainer          # 收 arXiv 論文，標為解說文
uv run knowfield ingest https://某部落格/attention-explained --explainer
uv run knowfield ask "transformer 為什麼用 attention"    # 種子立即可被問到（CLI＋web）
```
...
```bash
uv run knowfield pull "latent reasoning" --limit 20   # 預設每則消化成可讀散文
uv run knowfield pull "latent reasoning" --raw        # 純原礦：只給標題＋來源＋連結
uv run knowfield pull --from-digest 3                  # 從最近匯整第 3 則的主題深挖
```
...
```bash
uv run knowfield digest --format markdown            # 散文消化
uv run knowfield digest --format markdown --ai-image # 無原文圖時允許 AI 示意圖（會標「AI 示意・非原文」）
uv run knowfield digest --raw                        # 純原礦：只標題＋來源＋連結，不生成文字/圖
uv run knowfield digest --lang English               # 指定消化語言（預設繁體中文）
```
