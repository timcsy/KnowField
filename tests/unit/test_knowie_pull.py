"""`knowie-pull`／`knowie-xbase`：借來的判準**不能**被算成獨立撞到。
...
_XB = (pathlib.Path(__file__).resolve().parents[2]
       / "knowledge/skills/knowie-xbase/xbase.py")
_spec = importlib.util.spec_from_file_location("xbase", _XB)
xbase = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(xbase)
...
class TestEmitShape(unittest.TestCase):
    ...
    def test_block_says_how_to_promote(self):
        ...
        self.assertIn("真的撞到之後", xbase.emit_lesson(GROUP))
    ...
    def test_marker_appears_exactly_once(self):
        ...
        """
        self.assertEqual(xbase.emit_lesson(GROUP).count(xbase.BORROWED_MARK), 1)
...
class TestExclude(unittest.TestCase):
    def test_exclude_drops_that_base(self):
        ...
        found = xbase.find_bases([str(pathlib.Path(__file__).resolve().parents[3])])
