...
==================
版本變更: 1.1.0 → 1.2.0
變更類型: MINOR (放寬一項原則的約束範圍)

修改的原則:
  - VI. 使用者保有決策主權 — 理由句軟化。原「工具的職責是分診而非代替思考」隨知識庫
    原則 4 由「消化以省時，溯源不可省」再進一步為「消化到底，隨時可回溯」而放寬：
    消化／整理／跨來源 RAG 問答不再受限，主權改由「溯源可回＋否決權」保障。原則本身
    （可檢視/修改/覆寫）不變。來源：使用者 2026-07-24 要求鬆綁消化限制、加 RAG 問答；
    見 knowledge/history/016、knowledge/principles.md 原則 4。

--- 歷史 ---
版本變更: 1.0.0 → 1.1.0（MINOR，新增一項核心原則）
修改的原則:
  - (新增) VI. 使用者保有決策主權
...
理由：KnowField 的使用者是重度自我加工型讀者；工具負責**分診與消化以省時**（含
整理、跨來源 RAG 問答），把最終判斷與決策權保留給使用者——**溯源可回是主權的保障**：
使用者永遠能核對消化結果、跳回原文、覆寫工具的選擇。降低成本，而不剝奪判斷。
（2026-07-24 修訂：原句「工具的職責是分診而非代替思考」隨知識庫原則 4 由「消化以省時」
再進一步為「消化到底、隨時可回溯」而軟化——消化/合成不再受限，主權改由溯源與否決權
保障。見 `knowledge/principles.md` 原則 4、`knowledge/history/016`。）
