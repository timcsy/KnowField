"""知識的輸出（vision 階段 30 第一刀 B）：從場的理解生成「高證實文章」。

母概念：來源＝原文為真相、抽取為參考（`history/082`）的**輸出面**——文章＝理解的**再表達**。
守衛（vision 階段 30／原則 3、6）：
- **正文只採 🔬已證實＋🧩推論**；🌉類比／💭猜想 隔到「延伸閱讀」並標明（別當定論）。
- **溯源靠結構**（原則 3，不靠模型自律）：References 由程式從理解的佐證 url **確定性組**，
...
_SYS = (
    "你是把一個人自己冊封的『理解』寫成部落格教學文章的寫手。鐵律：\n"
    "1. **只根據下面編號的理解**寫，不要新增它們沒有的事實、不要杜撰。\n"
...
def build_article_prompt(topic: str, body: list, length: str = "medium",
    ...
    """給 LLM 的 user prompt：主題＋長度／難度＋編號的理解（含佐證脈絡）。"""
    lines = [f"主題：{topic}",
...
def generate_article(topic: str, nodes: list, chat_backend, embedder=None,
    ...
    """從已冊封理解生成高證實文章。回 {title, markdown, empty}。length/level＝長度/難度。
    body＝相關度前 top_k 的 🔬🧩；ext＝相關的 🌉💭（延伸閱讀）。References 結構化組（原則 3）。

    `pinned`（spec 043）＝某段對話冊封出的理解：**必被考慮**，排在最前。
