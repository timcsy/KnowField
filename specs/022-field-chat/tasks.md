...
- [X] T004 建 `src/learnnews/chat/field_chat.py`：`ChatBackend` Protocol＋`StubChatBackend`＋`build_field_system_prompt(roots)`（8 條膜指令＋場脈絡注入）＋`@dataclass CandidateDraft`＋`FieldChat.reply(history, user_msg, roots)`。跑 T001/T002/T003 轉綠。
...
- [X] T007 在 `src/learnnews/chat/field_chat.py` 加 `FieldChat.distill`（結構化蒸餾提示）；`src/learnnews/backends/openai_api.py` 加 `OpenAIChatBackend`（`_post` 多輪、poster 可注入）；`backends/factory.py` 加 `make_chat_backend(config)`。跑 T005/T006 轉綠。
...
- [X] T011 [US1] 在 `src/learnnews/web/app.py` 加 `app.state.chat_factory`（預設 `FieldChat.reply` with `make_chat_backend`＋`list_why_nodes('anointed')`）＋`distill_factory`＋`GET /chat`（頁）＋`POST /chat`（append→factory→重繪＋更新 hidden history）。跑 T008/T009 轉綠。
- [X] T012 [US3] 加 `POST /chat/distill`（→候選表單）＋`POST /chat/anoint`（`add_why_node`+`anoint`，人閘門；`_now_iso` 日期）。跑 T010 轉綠。
- [X] T013 [US1] 建 `src/learnnews/web/templates/chat.html`（桌面對話串：user/assistant 泡泡、hidden history JSON、輸入框；對話下「整理成冊封候選」→候選表單含「冊封」；每則 assistant 下「找佐證」鈕佔位）；`base.html` 導覽加「跟場聊」。
