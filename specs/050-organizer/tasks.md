...
## 追加：根領域（使用者回饋，2026-08-25）

> 「我覺得你現在做的還是很難整理，我希望原有的先被歸到**根領域**而不是未歸屬」

- [x] T021 `domain_id = null` 呈現為**樹頂的根領域**「🗂 知識庫」，不是樹外的桶子
      ⚠️ 資料模型一個字沒改——錯的是它的**位置與名字**
- [x] T022 FR-007 重新論證（**行為不變**）：糾纏是「兩件**刻意放過**的東西被拆散」，
      根領域是還沒放過的地方。把根算成位置的話，搬出第一件就對所有鄰居報糾纏
- [x] T023 領域樹置頂不捲走（清冊上百列，樹捲掉了就沒地方可以放）
- [x] T024 清冊搜尋框 ＋「全選這 N 件」——搜尋縮到一批之後逐件勾，就是 N×K 判準在打自己
- [x] T025 根領域也能「＋ 在這裡開新對話」
...
- [x] T026 實跑根領域：搜尋「Flow」98→6 件 → 全選 → 拖進 Flow Matching
      → 糾纏詢問只列出**在生成模型裡**的那 1 條（還在根領域的鄰居不算）
      → 「留一條糾纏」後：知識庫 98→92、Flow Matching 0→6 ✅
