...
**T001–T024 全部完成。** T024 於 2026-08-18 用瀏覽器實際跑過：切換兩個方向都正確
（繁體 ⇄ 原文），且所有 LaTeX 公式在兩種模式下都正常渲染——佔位保護在真實渲染管線下成立。
...
### ⚠️ T024 真跑照出一個缺陷（測試沒抓到）

**標題沒有被轉換**。實作只轉了 `markdown` 沒轉 `title`，但標題也是使用者在讀的內容（FR-001）。
19 個自動化測試全綠、覆蓋率看起來完整，卻沒有任何一條斷言標題——因為我寫測試時
**心裡的「內容」等於「正文」**，而使用者看到的內容包含標題。

修法：`/api/source` 對 `title` 一併套用（`raw=1` 時同樣回原文），並補契約測試釘住。
測試 419 → 420。

這條印證了 vision 階段 16 那個未結案的「真驗收（超越測試綠）」：
**測試綠只證明你想到的都對了，證明不了你沒想到的**。
